...
export async function POST(request: Request, { params }: { params: Promise<{ connectionId: string }> }) {
  ...
  try {
    ...
    const connection = await db.prepare(`SELECT id, name, base_url AS baseUrl, username, mode
    ...
    for (const installation of installations) {
      statements.push(db.prepare(`UPDATE hosting_domains SET wordpress_installation_id = COALESCE(?, wordpress_installation_id),
        ...
        wordpress_source = 'Softaculous operational connection'
    ...
    return json({
      ...
      message: `${String(connection.name)} is operational. Softaculous reported ${installations.length} installation${installations.length === 1 ? '' : 's'}, and ${defaultTemplateDomain} is ready as the default clone source.`,
    });
  } catch (error) {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ connectionId: string }> }) {
  ...
  await db.prepare(`UPDATE hosting_connections SET operational_auth_type = NULL,
    ...
    operational_credential_status = 'not_configured', mode = 'read_only', status = 'connected_read_only',
    write_actions_enabled = 0, destructive_actions_enabled = 0, updated_at = ?
