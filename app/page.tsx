      ...
      {wordpressAction && (
        <div className="project-modal-backdrop operation-confirm-backdrop" onClick={() => !wordpressActionBusy && setWordpressAction(null)}>
          <form className="operation-confirm-modal" onSubmit={runWordpressAction} onClick={(event) => event.stopPropagation()}>
            ...
            {wordpressAction.action === 'apply_php_profile' && <div className="info-callout">Checks and verifies both layers: cPanel PHP receives 768 MB memory, 512 MB post and upload sizes, 900-second execution and input times, and 5,000 input variables; WordPress receives at least 512 MB normal memory and 768 MB administrative memory in wp-config.php. Higher existing WordPress values are preserved, a dated rollback copy is made before changes, and primary public_html is excluded.</div>}
